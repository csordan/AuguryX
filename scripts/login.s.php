...
if (isset($_POST['login'])) {
  ...
  if (empty($email_) || empty($pwd_)) {
    header("Location: ../login.html?error=emptyfields&email=".$email_);
    exit();
  }
  else {
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
      ...
      mysqli_stmt_bind_param($stmt, "s", $email_);
      ...
      if ($row = mysqli_fetch_assoc($result)) {
        ...
        $pwdCheck = password_verify($pwd_, $row['pwdUser']);
        echo $row['pwdUser'];
        ...
        // Then if they DO match, then we know it is the correct user that is trying to log in!
          ...
          header("Location: ../account_success.html?login=success");
          exit();
        //}
      }
      else {
        header("Location: ../login.html?login=wronguidpwd");
        exit();
      }
    }
  }
